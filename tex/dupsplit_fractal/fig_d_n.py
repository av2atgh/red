...
def collect(qs, nnets):
    ...
    for N in [1000,10000,100000,1000000]:
        ...
        n.append(N)
        d.append(df.dmean[0])
    return pd.DataFrame(dict(n=n, d=d))
...
x = [min(x), max(x)]
ax[0].plot(x,np.exp(intercept + slope*np.log(x)), 'k--', label=r' $N^\alpha$, $\alpha=$'+f'{slope:.2f}')
...
x = df.n.values
y = df.d.values
ax[1].scatter(x, y, color='orange', label=r' $q=$'+f'{qs}')
slope, intercept, r, p, se = linregress(np.log(x), np.log(y))
x = [min(x), max(x)]
ax[1].plot(x,np.exp(intercept + slope*np.log(x)), 'k--', label=r' $N^\alpha$, $\alpha=$'+f'{slope:.2f}')
