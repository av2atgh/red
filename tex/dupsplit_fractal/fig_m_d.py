...
slope, intercept, r, p, se = linregress(np.log(df1.d.values), np.log(df1.m.values))
#ax[0].plot(x, np.exp(intercept + slope*np.log(x)), 'k:', label=f' $D_f$ ={slope:.2f}')
ax[0].plot(x, np.exp(1 + np.log(x)/0.55), 'k--', label=r' $D_f = \beta^{-1}$')
...
slope, intercept, r, p, se = linregress(np.log(df1.d.values), np.log(df1.m.values))
#ax[1].plot(x, np.exp(intercept + slope*np.log(x)), 'k:', label=f' $D_f$ ={slope:.2f}')
ax[1].plot(x, np.exp(3 + np.log(x)/0.22), 'k--', label=r' $D_f = \beta^{-1}$')

for i in [0,1]:
    ax[i].set(xlabel=r'$d$', ylabel=r'$\langle N\rangle_d$')
...
plt.savefig(f'/Users/avazquez/Dropbox/submissions/dupsplit_fractal_2023/fig_m_d.eps',bbox_inches='tight', facecolor='white', edgecolor='none', dpi=300)
