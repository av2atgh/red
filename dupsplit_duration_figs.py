...
for DMEAN in ["0"]:
    df = pd.read_csv(
        os.path.join(local, f"data/dupsplit_q{Q}_n{NNODES}_dmean{DMEAN}_i0_nodes.csv")
    )
...
plt.savefig(
    os.path.join(local, f"tex/dupsplit_duration/{fig_name}.pdf"),
    bbox_inches="tight",
    facecolor="white",
    edgecolor="none",
    dpi=300,
...
for Q in ["0.10", "0.30", "0.60"]:
    df = pd.read_csv(
        os.path.join(local, f"data/dupsplit_constant_q{Q}_n{NNODES}_i0_nodes.csv")
    )
...
ax[2].set(
    xlabel=r"$\gamma_T$", ylabel="Number of Projects", xticks=np.arange(-1, 4, step=1)
)
...
for project in projects[:3]:
    ...
    p_ = p_delay(d)
    r = np.where(
        r > 0, np.log(1 - p_[np.newaxis, :] + p_[np.newaxis, :] * LogNorm(r)), 0
    )
    pz = 1 - np.exp(np.sum(r, axis=1))
    ...
    ax[i].plot(
        ...
        pz,
        c="green",
        label=r"$P(d)\sim e^{-d/\langle d\rangle}$",
        linestyle=":",
    )
    ax[i].set(xlabel=r"$z$ (years)", ylabel=r"$Q(z)$", xticks=[0, 25, 50, 75, 100])
    ax[i].legend(loc="upper right", frameon=0, handletextpad=0, fontsize=12)
...
ax[i].plot(
    n,
    np.exp(intercept + slope * np.log(n)),
    linestyle="--",
    c="black",
    label=r"$n^{0.20}$",
)
...
slope, intercept, r, p, se = linregress(
    np.log(x[(y > 0) & (x > 5)]), np.log(y[(y > 0) & (x > 5)])
)
print(slope)
