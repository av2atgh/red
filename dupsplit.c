...
int main(int argc, char **argv) {
	...
	unsigned long d,sum,dmin=1,dmax=2;
	...
	for (net=0;net<n_nets;net++) {
		...
		for (d=dmin;d<=dmax;d++) {
			...
			mass+=nodes_at_distance_array[d]/n_nodes;
			x+=log(d+1);
			y+=log(mass);
			xy+=log(mass)*log(d+1);
			xx+=log(d+1)*log(d+1);
		...
		//
                for (i=0;i<=diameter;i++) ave_nodes_at_distance_array[i]+=nodes_at_distance_array[i]/n_nodes;
